Shader "Hidden/Natteens/Outline/Composite"
{
    SubShader
    {
        ...
        float3 ResolveDirect(int2 center)
        {
            ...
            if (current.a > 0.5)
                return float3(HasForeignNeighbor(center, current.rgb) ? 1.0 : 0.0, center);
            ...
            return float3(bestDistance < 100 ? 1.0 : 0.0, owner);
        }
        ...
        float4 DebugOutput(int2 center, bool outline)
        {
            ...
            if (_OutlineDebugMode == 2) value = visible.a > 0.5 ? visible.rgb : 0.0.xxx;
            if (_OutlineDebugMode == 3) value = (outline ? 1.0 : 0.0).xxx;
            if (_OutlineDebugMode == 4) value = (visible.a > 0.5 ? 1.0 : 0.0).xxx;
        ...
        float4 ResolveVisibility(Varyings input) : SV_Target
        {
            ...
            return selectedDepth <= sceneDepth + DepthTolerance(selectedDepth, sceneDepth) ? metadata : 0;
        }
        ...
        float4 CompositeDirect(Varyings input) : SV_Target
        {
            ...
            if (_OutlineDebugMode != 0)
                return DebugOutput(center, silhouette.x > 0.5);
